...
// Calculate the words-per-minute with the error adjustment

function calculateWPM() {
    const wordsTyped = totalTyped.trim().split(/\s+/).length;
    const baseWPM = Math.round((wordsTyped / 60) * 60);
    const adjustedWPNM = Math.max(baseWPM - errors, 0);
    return adjustedWPNM;

}

//Handle typing of the displayed text and scrolling....
